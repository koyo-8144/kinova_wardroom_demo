...
class CameraTFBroadcaster:
    ...
    ####### (camera calibration ->) broadcast Camera frame #######
    ...
    def start(self):
        ...
        if SET_START:
            ...
            rospy.loginfo("Calculate EE to Camera Frame")
            T_ee_camera_color, T_ee_camera_depth = self.calculate_ee_to_camera()
            rospy.loginfo("Publishing Camera Frames to tf")
            self.publish_tf_double(T_ee_camera_color, "/tool_frame", "/camera_color", T_ee_camera_depth, "/tool_frame", "/camera_depth")
    ...
    def calculate_ee_to_camera(self):
        ...
        return T_ee_camera_color, T_ee_camera_depth
    ...
    def publish_tf_double(self, T_1, parent_frame_1, child_frame_1, T_2, parent_frame_2, child_frame_2):
        """ Continuously broadcast the transformation """
        # Extract translation (last column of the first 3 rows)
        translation_1 = T_1[:3, 3]
        # Extract rotation matrix (upper-left 3×3)
        rotation_matrix_1 = T_1[:3, :3]
        # Convert rotation matrix to quaternion
        quaternion_1 = tf.transformations.quaternion_from_matrix(T_1)

        # Extract translation (last column of the first 3 rows)
        translation_2 = T_2[:3, 3]
        # Extract rotation matrix (upper-left 3×3)
        rotation_matrix_2 = T_2[:3, :3]
        # Convert rotation matrix to quaternion
        quaternion_2 = tf.transformations.quaternion_from_matrix(T_2)


        rospy.loginfo(f"Publishing transformation: {parent_frame_1} → {child_frame_1}")
        rospy.loginfo(f"Translation: {translation_1}")
        rospy.loginfo(f"Quaternion: {quaternion_1}")

        rospy.loginfo(f"Publishing transformation: {parent_frame_2} → {child_frame_2}")
        rospy.loginfo(f"Translation: {translation_2}")
        rospy.loginfo(f"Quaternion: {quaternion_2}")

        while not rospy.is_shutdown():
            self.broadcaster.sendTransform(
                translation_1,    # Position (x, y, z)
                quaternion_1,     # Orientation (x, y, z, w)
                rospy.Time.now(),
                child_frame_1,  # Child frame
                parent_frame_1      # Parent frame
            )

            self.broadcaster.sendTransform(
                translation_2,    # Position (x, y, z)
                quaternion_2,     # Orientation (x, y, z, w)
                rospy.Time.now(),
                child_frame_2,  # Child frame
                parent_frame_2      # Parent frame
            )
